...
\begin{table}
  ...
  \begin{tabular}{p{0.2cm} p{6cm} p{12cm} p{6cm} p{4cm}}
    ...
    \\ \\ \\
    2  &  Backpropagation
      & $\displaystyle \frac{\partial L}{\partial \theta} = \frac{\partial L}{\partial z}\frac{\partial z}{\partial \theta} $      & Computing Gradients      & \cite{rnn_backprop}
    \\ \\ \\
    3  &  Linear Layer
    ...
    \\ \\ \\
    5  &  ReLU
      & $\displaystyle \operatorname{ReLU}\left( x \right) = \max(0, x)$      & Activation Function      & \cite{relu}
    \\ \\ \\
    6  &  Recurrent Cells
      & $\displaystyle h_{t+1} = \tanh \left( W_{ih} x_t + b_{ih} + W_{hh}h_t + b_{hh} \right)$      & Sequence Modeling      & \cite{rnn_backprop}
    \\ \\ \\
    7  &  Self Attention
